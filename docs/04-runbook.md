...
```bash
DEV '... && rosrun path_tracking lap_logger.py _out:=/tmp/lap.csv _start_idx:=972'
```

`_start_idx` 는 대회 라인이 시작하는 인덱스(=이음매)다. 경로를 다시 딸 때마다
바뀌므로 `path_join.py` 가 출력하는 "이음매 위치" 를 보고 넣는다.

기준값 (2026-07-29 재기록 경로, 크루즈 55km/h)
...
| 차로여유 0.654m 초과 | 0건 |
| 0.4m 초과 | 5개 (0.34%) |
| 완주 | 2830m / 240초 / 평균 42.4km/h |

참고로 초안 경로는 평균 0.149m, 최대 0.429m 로 사실상 같았다. 재기록으로
경로의 기하학적 거칠기는 24% 줄었지만 CTE 는 변하지 않았다. 초안도 이미
추종 여유 안에 있었기 때문이다. 재기록의 가치는 CTE 가 아니라 "차로 안에서
경로가 어디에 그어졌는가" 에 있고, 그것은 CTE 로 잴 수 없다.
